<template>
  <el-col :span="20">
    ...
    >
      ...
      <el-table-column label="名称" prop="movie_name"></el-table-column>
...
export default {
  ...
  mounted () {
    ...
    config.GetPageInfo(this.entity, { pageNum: 1, pageSize: 5, key: '' }, this.success)
  },
  computed: mapState({
    ...mapGetters('loading', {
      loading: 'getLoading',
  ...
  methods: {
    ...
    keyChange (e) {
      this.key = e
      config.GetPageInfo(this.entity, { pageNum: 1, pageSize: this.pageSize, key: this.key }, this.success)
    },
    /* 表格搜索关键字 */
    tableKeyChange (e) {
      this.tableKey = e
    ...
    add () {
      ...
      this.$router.push({ path: '/Movie/Editing' })
    },
    /* 成功 */
    success (res) {
    ...
    handleSizeChange (pageSize) {
      ...
      config.GetPageInfo(this.entity, { pageNum: 1, pageSize: pageSize, key: '' }, this.success)
    },
    /* 当前页数改变 */
    handleCurrentChange (currentPage) {
      this.$store.commit('pagination/SET_CURRENT_PAGE', currentPage)
      config.GetPageInfo(this.entity, { pageNum: currentPage, pageSize: this.pageSize, key: '' }, this.success)
    },
    /* 编号 */
    indexMethod (index) {
    ...
    handleEdit (index, row) {
      this.$store.commit('movie/SET_MOVIE', row)
      //  编辑action 置为 1
    ...
    handleDelete (index, row) {
      this.$confirm('此操作将永久删除该电影信息, 是否继续?', '提示', {
        ...
        config.DeleteOne(this.entity, { id: row.id }, () => {
          config.GetPageInfo(this.entity, { pageNum: this.currentPage, pageSize: this.pageSize, key: '' }, this.success)
          this.$notify({
            type: 'success',
            message: '删除成功!'
          })
